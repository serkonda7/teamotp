...
| - | ----- | ----- | ------ | -------- |
| 1 | Runtime validation at route boundaries | secret overwrite, PK rewrite, invalid secrets | M | no |
...
## Phase 1 — Runtime validation at route boundaries

**Problem.** `UpdateOtpEntry` is documented as "Other fields are not updatable"
(`server/src/types.ts:14`) but `POST /otp/:id` casts the body and passes it straight
to `db.update(entries).set(body)` (`server/src/routes/otp_routes.ts:71-82`). Probes
confirmed a single request can rewrite `secret`, `digits`, `period`, `archived_at`,
and even the primary key `id`. Separately, `POST /otp` accepts a non-base32 secret
with **201**, after which every code fetch returns **500**.

### 1.1 Add shared schemas

Add `valibot` to the `shared` workspace and create `shared/src/schemas.ts`. Put the
data contracts there and derive the TypeScript types from them, so the runtime
contract and the compile-time type can never drift again:
...
export const NewOtpEntrySchema = v.object({
	...
	algorithm: v.optional(v.picklist(['sha1', 'sha256', 'sha512'])),
	digits: v.optional(v.picklist([6, 7, 8])),
...
export const NewTagSchema = v.object({
	...
	color: v.pipe(v.string(), v.regex(/^#[0-9a-fA-F]{6}$/), v.toLowerCase()),
})

export type NewOtpEntry = v.InferOutput<typeof NewOtpEntrySchema>
export type UpdateOtpEntry = v.InferOutput<typeof UpdateOtpEntrySchema>
export type NewTag = v.InferOutput<typeof NewTagSchema>
```

Then delete the hand-written `NewOtpEntry` / `NewTag` interfaces from
`shared/src/types.ts` and `UpdateOtpEntry` from `server/src/types.ts`, re-exporting
from `schemas.ts` instead. Keep `OtpDisplayInfo`, `TagInfo`, `TagWithMemberCount`
and the JSX helper types in `types.ts` — those are response/UI shapes, not inputs.

> `valibot` becomes a client dependency via `shared`. It is tree-shakeable and only
> the used validators ship, but check the bundle delta after this phase. If it is
> unacceptable, move `schemas.ts` into `server/src/validation/` instead and accept
> that the client keeps its own duplicate types.

### 1.2 Wire the validator into the routes

Add `@hono/valibot-validator` (`0.6.1`, verified available) to the catalog in the
root `package.json` and to `server`. It plugs into Hono's RPC type inference, so
`client/src/api.ts` gets the validated input types for free:

```ts
import { vValidator } from '@hono/valibot-validator'

.post('/', vValidator('json', NewOtpEntrySchema), (c) => {
	const entry = createEntry(c.req.valid('json'))
	return c.json({ id: entry.id }, 201)
})

.post('/:id', vValidator('json', UpdateOtpEntrySchema), (c) => { /* ... */ })
```

Apply to `POST /otp`, `POST /otp/:id`, and `POST /tags`. Add a shared error hook so
validation failures return the existing `{ error: string }` shape rather than
valibot's default issue array — `client/src/util/api_error.ts` reads `.error`.

Drop the now-dead hand-rolled checks in `tag_routes.ts:25-36` and
`otp_routes.ts:37-40`, and remove the `// TODO server-side validation` comment.

### 1.3 Harden `updateEntry` at the data layer too

Route-level parsing is the primary fix, but it only protects the one call site that
remembers to use it. `updateEntry` (`db.ts:121-123`) currently spreads whatever the
caller hands it straight into `.set()`, so it stays a mass-assignment primitive for
any future caller. Make the data layer enforce the whitelist independently:
...
export function updateEntry(id: string, updated: UpdateOtpEntry): void {
	...
	if (updated.label !== undefined) fields.label = updated.label
	if (updated.issuer !== undefined) fields.issuer = updated.issuer
	if (updated.issuer_second !== undefined) fields.issuer_second = updated.issuer_second
...
This also absorbs the empty-body guard currently sitting in the route
(`otp_routes.ts:79-82`), so that check can be deleted along with the hand-rolled
validation.

> Both layers matter for different reasons: `v.strictObject` rejects the request
> loudly with a 400 so a caller learns they did something wrong, while the data layer
> guarantees the column can't be written even if a route forgets to validate. Keep
> both; they are not redundant.

### 1.4 Verify the secret before it is stored

Schema validation catches malformed Base32, but the guardrails in `otp.ts` can still
reject an input that passes the regex. Make `createEntry` prove the secret works:

- In `POST /otp`, after building the entry and **before** `db.insert`, call
  `generateTotpCode` once. On error return **400** with the message instead of
  storing a row that permanently 500s on read.

### 1.5 Tests

Add to `server/src/routes/otp_routes.test.ts`:

- `POST /otp/:id` with `{ secret, digits, period, archived_at }` → **400**, and
  `getEntryById` shows all original values unchanged.
- `POST /otp/:id` with `{ id: 'hijacked' }` → **400**, original id still resolves.
- `POST /otp` with `secret: '!!!!not-base32!!!!'` → **400**, no row created.
- `POST /otp` with a lowercase secret → **201**, stored uppercased (existing
  behaviour, now enforced by the schema transform).
- Unit test on `updateEntry` directly (bypassing the route) called with
  `{ label: 'x', secret: 'EVIL' } as UpdateOtpEntry` → the secret column is
  unchanged. This is the test that proves the data-layer guard, not the route.
